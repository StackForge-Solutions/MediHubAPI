...
WHERE a.appointmentDate BETWEEN :start AND :end
  ...
  AND (
      :q IS NULL OR :q = ''
      OR LOWER(CONCAT(a.patient.firstName, ' ', a.patient.lastName))
            LIKE LOWER(CONCAT('%', :q, '%'))
      OR LOWER(CONCAT(a.doctor.firstName, ' ', a.doctor.lastName))
            LIKE LOWER(CONCAT('%', :q, '%'))
      OR STR(a.patient.id) LIKE CONCAT('%', :q, '%')
      OR STR(a.id) LIKE CONCAT('%', :q, '%')
  )
"""
            ,
            countQuery = """
